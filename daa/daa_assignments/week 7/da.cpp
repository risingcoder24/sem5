...
int main() { int G[max][max]={{0,1,2,0,0,0},{1,0,1,3,2,0},{2,1,0,1,2,0},{0,3,1,0,4,3},{0,0,2,4, 0,3},{0,0,0,3,3,0}};
 int n=6; int u=0; dijkstra(G,n,u);
 } 
 void dijkstra(int G[max][max],int n,int startnode) 
 { int cost[max][max],distance[max],pred[max]; 
 int visited[max],count,mindistance,nextnode,i,j; 
 for(i=0;i<n;i++) for(j=0;j<n;j++) if(G[i][j]==0) cost[i][j]=INFINITY; 
 else cost[i][j]=G[i][j]; for(i=0;i<n;i++) 
 ...
 distance[startnode]=0; visited[startnode]=1; count=1; while(count<n-1)
  ...
  cout<<"\tPath="<<i; j=i; do { j=pred[j]; 
  cout<<"<-"<<j; }while(j!=startnode); } }
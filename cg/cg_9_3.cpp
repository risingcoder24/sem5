...
int visibility(PT p1, PT p2); PT resetendpt(PT p1, PT p2); void drawwindow()
{
setcolor(WHITE);
...
ptemp.code[0] = '0'; if (p.y >= 400)
ptemp.code[1] = '1'; else
ptemp.code[1] = '0'; if (p.x >= 450)
ptemp.code[2] = '1'; else
ptemp.code[2] = '0';
 
if (p.x <= 150) ptemp.code[3] = '1';
else
ptemp.code[3] = '0'; ptemp.x = p.x; ptemp.y = p.y;
return (ptemp);
}
midsub(PT p1, PT p2)
{
PT mid; int v;
setcolor(YELLOW); p1 = setcode(p1);
p2 = setcode(p2);
...
printf("Enter 1st endpoint (x,y):"); scanf("%d %d", &p1.x, &p1.y); printf("Enter 2nd endpoint (x,y):"); scanf("%d %d", &p2.x, &p2.y); drawwindow();
midsub(p1, p2); getch(); closegraph();
